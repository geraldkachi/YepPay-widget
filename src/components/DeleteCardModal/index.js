...
import { removeRememberedCard } from '../../services/card';
import withInitiatePayment from '../HOC/withInitiatePayment';
import ActionButton from '../Button/ActionButton';
import { useQueryClient } from 'react-query';
import { GET_REMEMBERED_CARDS } from '../../utils/constants/queryTypes';

const DeleteCardModal = ({ setCardToDelete, card, paymentDetail }) => {
  const [loading, setLoading] = useState(false);
  const queryClient = useQueryClient();

  const handleRemoveCard = async () => {
    setLoading(true);
    const response = await removeRememberedCard({
      business_id: paymentDetail.business?.id,
      email: paymentDetail.customer?.email,
      first_6: card.first_6,
      last_4: card.last_4,
    });
    setLoading(false);
    if (response.status) {
      toast.success(response.message);
      await queryClient.refetchQueries(GET_REMEMBERED_CARDS);
      return setCardToDelete();
    } else {
    ...
    <div id="deleteCardModal" className="modal">
      <div className="modal-content">
        ...
        <div className="modal-content-footer">
          <WidgetFooter verb="Dismiss" onClick={setCardToDelete} />
        </div>
      </div>
    </div>
  );
...
export default withInitiatePayment(DeleteCardModal);
